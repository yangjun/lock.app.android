...
public class SlideSwitch extends View {
	...
	private static final long CLICK_INTERVAL = 200L;
	
	public static final int SHAPE_RECT = 1;
	public static final int SHAPE_CIRCLE = 2;
	private static int RIM_SIZE;
	private static final int DEFAULT_COLOR_THEME = Color.parseColor("#ff00ee00");
	private static final int DEFAULT_COLOR_THEME_BACK = Color.GRAY;

	// 3 attributes
	private int color_theme;
	private int color_theme_back;
	...
	public SlideSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		color_theme = a.getColor(R.styleable.slideswitch_themeColor,
				DEFAULT_COLOR_THEME);
		color_theme_back = a.getColor(R.styleable.slideswitch_themeColorBackground,
				DEFAULT_COLOR_THEME_BACK);
		isOpen = a.getBoolean(R.styleable.slideswitch_isOpen, false);
		shape = a.getInt(R.styleable.slideswitch_shape, SHAPE_RECT);
		
		frontRect_left_begin = RIM_SIZE = dip2px(context, 1.0f);
		a.recycle();
	}
	...
	protected void onDraw(Canvas canvas) {
		if (shape == SHAPE_RECT) {
			paint.setColor(color_theme_back);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if (System.currentTimeMillis() - mDownTime > CLICK_INTERVAL) {
	...
	@SuppressLint("NewApi") public void moveToDest(final boolean toRight) {
		final boolean initSlideable = slideable;
		setSlideable(false);
