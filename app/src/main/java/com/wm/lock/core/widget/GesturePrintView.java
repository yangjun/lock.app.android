...
public class GesturePrintView extends ViewGroup {
     ...
     * 是否是抬起手势
     */
    private boolean isGestureUp = false;

    /**
     * 至少连接的点数量
     */
    private int minConnectPoints = 1;
    ...
    public void setMinConnectPoints(int minConnectPoints) {
        this.minConnectPoints = minConnectPoints;
    }
    ...
    public GesturePrintView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public GesturePrintView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public GesturePrintView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr);
        initFromAttributes(attrs, defStyleAttr);
    }

    /**
     * 重置。如果password不为空，表示是一次验证；否则就是设置，需要两次验证。
     * @param password
     */
    ...
    private void initFromAttributes(AttributeSet attrs, int defStyleAttr) {
        // 获取定义的属性
        final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.GesturePrintView, defStyleAttr, 0);

        nodeSrc = a.getDrawable(R.styleable.GesturePrintView_lock9_nodeSrc);
        nodeOnSrc = a.getDrawable(R.styleable.GesturePrintView_lock9_nodeOnSrc);
        lineColor = a.getColor(R.styleable.GesturePrintView_lock9_lineColor, Color.argb(0, 0, 0, 0));
        lineWidth = a.getDimension(R.styleable.GesturePrintView_lock9_lineWidth, 0);
        padding = a.getDimension(R.styleable.GesturePrintView_lock9_padding, 0);
        spacing = a.getDimension(R.styleable.GesturePrintView_lock9_spacing, 0);
...
//            int nodeWidth = (int) ((right - left - padding * 2 - spacing * 2) / 3);
            int nodeWidth = (int) ((right - left - spacing * 2) / 3);
            for (int n = 0; n < 9; n++) {
...
//                int t = (int) (padding + row * (nodeWidth + spacing));
                int l = (int) (col * (nodeWidth + spacing));
                int t = (int) (row * (nodeWidth + spacing));
                int r = l + nodeWidth;
                int b = t + nodeWidth;
                node.layout(l, t, r, b);
            }
...
//                    return true;
//                }
                if (newPassword.length() < minConnectPoints) {
                    callBack.onFail(Error.SHORT);
                    invalidate(); //去掉拖动线
                    clearLine();
                    return true;
                }
                ...
                if (aleradyCount >= 2) {
                    ...
                    callBack.onRepeat();
                    lastPassword = SecurityManager.md5(newPassword);
                    invalidate(); //去掉拖动线
                    clearLine();
                }
                break;
    ...
    private void verify(String comparePassword) {
        String newPassword = SecurityManager.md5(passwordBuilder.toString());
        if (comparePassword.equals(newPassword)) {
            ...
            invalidate(); //去掉拖动线并置为红色
            callBack.onFail(Error.NOT_MATCH);
        }
        clearLine();
        setValidPassword(validPassword); //重置
    }

    private Runnable clearLineRunnable = new Runnable() {
        @Override
        public void run() {
            // 清空状态
            lineList.clear();
    ...
    private void clearLine() {
        mHandler.postDelayed(clearLineRunnable, 300);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 如果已经有点亮的点，且还没有抬起手势
        if (currentNode != null && !isGestureUp) {
